        ...
        listOf(LaunchMethod.Web, LaunchMethod.Wap).forEach { launch ->
            ...
            listOf(Language.EN, Language.CN).forEach { language ->
                val games = gameApi.slotGames(clientId = 1, platform = Platform.Pragmatic, launch = launch, language = language)
                        .filter { pragmaticMap[it.gameId] != null }
                        .map {
                            if (language == Language.CN) {
                                val gameName = pragmaticMap[it.gameId]
...
//            }
                this.upload2(games = sortGames.subList(0, 10), path = "slot/hot_sort_10_${launch.name.toLowerCase()}_${language.name.toLowerCase()}.json")
                this.upload2(games = sortGames.subList(0, 15), path = "slot/hot_sort_15_${launch.name.toLowerCase()}_${language.name.toLowerCase()}.json")
                this.upload2(games = sortGames.subList(0, 20), path = "slot/hot_sort_20_${launch.name.toLowerCase()}_${language.name.toLowerCase()}.json")
            }
    ...
    fun jokerGameTask() {
        ...
        val webGames = gameApi.slotGames(clientId = 1, platform = Platform.Joker, launch = LaunchMethod.Web, language = Language.EN)
        val games = SlotMenuUtil.addCategory(webGames, SlotMenuUtil.jokerJson)


        val file = File("/Users/cabbage/Desktop/joker_china.csv")
        val map = file.readLines().map { line ->
            val data = line.split(",")
            data[0] to data[1]
        }.toMap()

        val cnGames = games.map {
            it.copy(gameName = map[it.gameId]!!)
        }


        this.upload(games = games, path = "slot/joker_en.json")
        this.upload(games = cnGames, path = "slot/joker_cn.json")
    ...
    fun pragmaticTask() {
        ...
        listOf(LaunchMethod.Web, LaunchMethod.Wap).forEach { launch ->
            ...
            listOf(Language.EN, Language.CN).forEach { language ->
                ...
                this.upload(games = games2, path = "slot/pragmatic_${launch.name.toLowerCase()}_${language.name.toLowerCase()}.json")
            }
...
//            file.delete()
    ...
    fun ttgGameTask() {
        ...
        val webGames = gameApi.slotGames(clientId = 1, platform = Platform.TTG, launch = LaunchMethod.Web, language = Language.EN)
        ...
        val wapGames = gameApi.slotGames(clientId = 1, platform = Platform.TTG, launch = LaunchMethod.Wap, language = Language.EN)
