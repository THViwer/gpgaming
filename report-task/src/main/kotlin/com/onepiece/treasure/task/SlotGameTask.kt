...
//    }
     ...
     * 生成热门游戏
     */
    fun generatorHotGames(language: Language) {

        listOf(LaunchMethod.Web, LaunchMethod.Wap).forEach {
            val games = gameApi.slotGames(clientId = 1, platform = Platform.Pragmatic, launch = it, language = language)
...
//            }
            this.upload2(games = sortGames.subList(0, 10), path = "slot/hot_sort_10_${it.name.toLowerCase()}_${language.name.toLowerCase()}.json")
            this.upload2(games = sortGames.subList(0, 15), path = "slot/hot_sort_15_${it.name.toLowerCase()}_${language.name.toLowerCase()}.json")
            this.upload2(games = sortGames.subList(0, 20), path = "slot/hot_sort_20_${it.name.toLowerCase()}_${language.name.toLowerCase()}.json")
        }
    }
