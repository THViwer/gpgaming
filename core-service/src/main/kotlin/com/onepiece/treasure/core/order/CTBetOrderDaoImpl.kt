...
@Repository
class CTBetOrderDaoImpl : BasicDaoImpl<CTBetOrder>("ct_bet_order"), CTBetOrderDao {
    ...
    override fun report(startDate: LocalDate, endDate: LocalDate): List<BetOrderValue.Report> {
        val sql = """
            select 
                client_id, member_id, sum(bet_points) as bet, sum(win_or_loss) as win from ct_bet_order 
            where 
