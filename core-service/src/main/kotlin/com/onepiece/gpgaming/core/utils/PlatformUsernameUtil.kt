...
//            }
    ...
    fun prefixPlatformUsername(platform: Platform, platformUsername: String, prefix: String = " "): Pair<Int, Int> {
        ...
        val cutSize = this.cutSize(platform = platform)
        return when (platform) {
            Platform.GoldDeluxe -> {
                val clientId = platformUsername.substring(1, cutSize).toInt()
                val memberId = platformUsername.substring(cutSize, platformUsername.length).toInt()
...
//        }
    ...
    private fun cutSize(platform: Platform): Int {
        return when (ActiveUtil.active) {
            "dev",
            "sit",
            "prod" -> {
                if (platform == Platform.GoldDeluxe) 3 else 2
            }
            // prodS2、prodS3
            else -> {
                if (platform == Platform.GoldDeluxe) 6 else 5
            }
        }
    }
